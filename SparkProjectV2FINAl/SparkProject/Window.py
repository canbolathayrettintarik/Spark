...
class BlueScanGUI:
    ...
    def _build_ui(self):
        ...
        self.ua_label = ctk.CTkLabel(self.sidebar, text="Custom User-Agent:", font=("Urbanist", 12, "bold"))
        self.ua_label.pack(pady=(15, 2), padx=20, anchor="w")
        
        self.ua_entry = ctk.CTkEntry(self.sidebar, placeholder_text="Varsayılan (Rastgele)", width=180)
        self.ua_entry.pack(pady=2, padx=20)

        self.fp_label = ctk.CTkLabel(self.sidebar, text="TLS Fingerprint (JA3):", font=("Urbanist", 12, "bold"))
        self.fp_label.pack(pady=(10, 2), padx=20, anchor="w")
        
        self.fp_option = ctk.CTkOptionMenu(
            self.sidebar, 
            values=["Default", "chrome120", "chrome119", "firefox120", "safari17"], 
            width=180,
            fg_color="#2b2b2b",
            button_color="#4a4a4a"
        )
        self.fp_option.pack(pady=2, padx=20)
        self.fp_option.set("Default")
    ...
    def run_scanner(self, target, port_list):
        ...
        raw_ua = self.ua_entry.get().strip()
        user_agent_val = raw_ua if raw_ua else None
        
        raw_fp = self.fp_option.get()
        fingerprint_val = None if raw_fp == "Varsayılan" else raw_fp
